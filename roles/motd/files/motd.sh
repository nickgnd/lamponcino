...
( : '~'.~.'~' : )   Load Averages:      ${one}, ${five}, ${fifteen} (1, 5, 15 min)
 ~ .~ (   ) ~. ~    Running Processes:  $(/bin/ps ax | /usr/bin/wc -l | /usr/bin/tr -d " ")
  (  : '~' :  )     IP Addresses:       $(/sbin/ifconfig wlan0 | /bin/grep inet | head -n 1 | /usr/bin/tr -s " " | /usr/bin/cut -d " " -f 3) and $(/usr/bin/curl -s --connect-timeout 1 http://ipinfo.io/ip | /usr/bin/tail)
